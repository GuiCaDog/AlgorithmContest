        ...
        if left == INF and right == INF:
            ...
            if left > right or left > r or right < l:#no overlap
                ...
                if v == 'z':
                    ...
                    tree[node] = max(0,tree[node] + v*(right - left + 1))
...
if __name__ == '__main__':
    ...
    for case in range(1, case_count + 1):
        ...
        for i in range(0, k):
            ...
            queries.append(([querie[0]] + [int(querie[i]) for i in range(1, len(querie))]))
        ...
        for q in queries:
            if q[0] == 's':
                rangeAdd(tree, lazy,n, q[1]-1, q[1]-1, 1)
            elif q[0] == 'd':
                rangeAdd(tree, lazy,n, q[1]-1, q[1]-1, -1)
            else:
                r += rangePeek(tree, lazy, n, q[1]-1, q[2]-1)
                rangeAdd(tree, lazy,n, q[1]-1, q[2]-1, 'z')
